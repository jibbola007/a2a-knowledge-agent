...
var MastraScorer = class _MastraScorer {
  ...
  toMastraWorkflow() {
    const workflowSteps = this.steps.map((scorerStep) => {
      return chunk5KNLBUBH_cjs.createStep({
        id: scorerStep.name,
    ...
    const workflow = chunk5KNLBUBH_cjs.createWorkflow({
  ...
  async executePromptStep(scorerStep, tracingContext, context) {
    ...
    const resolvedModel = await chunk7J3XX4AO_cjs.resolveModelConfig(modelConfig);
    const judge = new chunk5KNLBUBH_cjs.Agent({
      name: "judge",
      model: resolvedModel,
      instructions,
    ...
    if (scorerStep.name === "generateScore") {
      ...
      if (resolvedModel.specificationVersion === "v2") {
        result = await chunk5KNLBUBH_cjs.tryGenerateWithJsonFallback(judge, prompt, {
          structuredOutput: {
            schema
          },
          tracingContext
...
function isWorkflow(target) {
  return target instanceof chunk5KNLBUBH_cjs.Workflow;
}
...
//# sourceMappingURL=chunk-MXJRZZEC.cjs.map
//# sourceMappingURL=chunk-MXJRZZEC.cjs.map