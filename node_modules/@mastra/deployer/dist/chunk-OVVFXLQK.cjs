    ...
    resolveId(id, importer) {
      ...
      try {
        ...
        for (const ext of [".mjs", ".js", ".cjs"]) {
          ...
          if (resolved2) {
            const pkgName = chunkWVZENB5V_cjs.getPackageName(id);
            if (!pkgName) {
              return null;
            }
...
async function getInputOptions2(entryFile, platform, env, { sourcemap = false } = {}) {
  ...
  const projectRoot = closestPkgJson ? path.dirname(chunkWVZENB5V_cjs.slash(closestPkgJson)) : chunkWVZENB5V_cjs.slash(process.cwd());
  const { workspaceMap, workspaceRoot } = await chunkWVZENB5V_cjs.getWorkspaceInformation({ mastraEntryFile: entryFile });
  const analyzeEntryResult = await chunkWVZENB5V_cjs.analyzeBundle(
    [entryFile],
    entryFile,
    {
  ...
  for (const [dep, metadata] of analyzeEntryResult.dependencies.entries()) {
    const pkgName = chunkWVZENB5V_cjs.getPackageName(dep);
  ...
  if (Array.isArray(inputOptions.plugins)) {
    ...
    inputOptions.plugins.forEach((plugin) => {
      ...
      if (plugin?.name === "tsconfig-paths") {
        plugins.push(
          chunkWVZENB5V_cjs.tsConfigPaths({
            localResolve: true
          })
        );
        return;
    ...
    inputOptions.plugins.push(chunkWVZENB5V_cjs.aliasHono());
...
function removeAllOptionsExceptServer(result, logger) {
  return chunkWVZENB5V_cjs.removeAllOptionsFromMastraExcept(result, "server", logger);
}
...
//# sourceMappingURL=chunk-OVVFXLQK.cjs.map
//# sourceMappingURL=chunk-OVVFXLQK.cjs.map