...
var Bundler = class extends bundler.MastraBundler {
  ...
  async writeInstrumentationFile(outputDirectory, customInstrumentationFile) {
    ...
    const __dirname = path.dirname(url.fileURLToPath((typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === 'SCRIPT' && _documentCurrentScript.src || new URL('chunk-7TQIPPJV.cjs', document.baseURI).href))));
  ...
  createBundler(inputOptions, outputOptions) {
    return chunkNMOPG6KM_cjs.createBundler(inputOptions, outputOptions);
  }
  async analyze(entry, mastraFile, outputDirectory, { enableEsmShim = true } = {}) {
    return await chunkWVZENB5V_cjs.analyzeBundle(
      [].concat(entry),
      mastraFile,
      {
  ...
  async getBundlerOptions(serverFile, mastraEntryFile, analyzedBundleInfo, toolsPaths, { enableSourcemap = false, enableEsmShim = true } = {}) {
    const { workspaceRoot } = await chunkWVZENB5V_cjs.getWorkspaceInformation({ mastraEntryFile });
    ...
    const inputOptions = await chunkNMOPG6KM_cjs.getInputOptions(
      mastraEntryFile,
      analyzedBundleInfo,
      "node",
      {
  ...
  async _bundle(serverFile, mastraEntryFile, {
    ...
    let sourcemap = false;
    try {
      const bundlerOptions = await chunkWVZENB5V_cjs.getBundlerOptions(mastraEntryFile, analyzeDir);
    ...
    try {
      ...
      analyzedBundleInfo = await chunkWVZENB5V_cjs.analyzeBundle(
    ...
    try {
      const result = await chunkHPIEW72C_cjs.writeTelemetryConfig(mastraEntryFile, path.join(outputDirectory, this.outputDir));
    ...
    for (const dep of analyzedBundleInfo.externalDependencies) {
      try {
        ...
        const rootPath = await chunkWVZENB5V_cjs.getPackageRootPath(dep);
...
//# sourceMappingURL=chunk-7TQIPPJV.cjs.map
//# sourceMappingURL=chunk-7TQIPPJV.cjs.map